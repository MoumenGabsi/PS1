"""
Categories Tab - CRUD interface for categories (CustomTkinter version)
"""
...
class CategoriesTab(ctk.CTkFrame):
    ...
    def __init__(self, parent):
        ...
        self.repo = CategoriesRepository()
    ...
    def _add_table_row(self, category_id, name, description):
        """Add a row to the categories table"""
        ...
        # Store category ID for operations
        row_frame.category_id = category_id
        
        values = [str(category_id), name, description]
        widths = [30, 150, 400]
    ...
    def _on_add(self):
        ...
        dialog = AddUpdateDialog(self, "Add Category", fields)
        result = dialog.get_result()
        
        if result:
            is_valid, error = self.validator.validate_category_form(
                result['Name'],
                result['Description']
            )
            ...
            category_id = self.repo.create(
                result['Name'],
                result['Description']
            )
            
            if category_id:
                SuccessDialog.show(self, "Success", "Category added successfully")
                self._load_data()
            else:
                ErrorDialog.show(self, "Error", "Failed to add category")
    ...
    def _on_edit(self):
        ...
        category_id = self.selected_row.category_id
        
        category = self.repo.read(category_id)
        if not category:
            ErrorDialog.show(self, "Error", "Category not found")
            return
        ...
        dialog = AddUpdateDialog(self, "Edit Category", fields, initial)
        result = dialog.get_result()
        ...
        if result:
            ...
            if success:
                SuccessDialog.show(self, "Success", "Category updated successfully")
                self._load_data()
            else:
                ErrorDialog.show(self, "Error", "Failed to update category")
    ...
    def _on_delete(self):
        ...
        if DeleteConfirmDialog.show(self, "Delete Category", f"Category: {category_name}"):
            success = self.repo.delete(category_id)
            
            if success:
                SuccessDialog.show(self, "Success", "Category deleted successfully")
                self._load_data()
            else:
